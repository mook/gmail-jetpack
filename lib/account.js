...
const Account = exports.Account = Class({
    ...
    login: function(callback) {
        ...
        deferred.promise.then((logins) => {
            ...
            return RequestPromise({
                url: new URL(form.getAttribute("action"), this._loginURL),
            ...
            if (!cookies.some((cookie) => cookie.name == "SID")) {
                // Nope, login failed, no session id
                throw "Login failed";
            }
            return doc;
        }).then((doc) => {
    ...
    check: function() {
        RequestPromise({
            ...
            console.log("Got document:", doc.body.innerHTML);
        }).then(null, (error) => {
            console.exception(error);
            this._on_logout();
        });
     ...
     */
    get _loginURL() URL("https://accounts.google.com/ServiceLoginAuth"),
