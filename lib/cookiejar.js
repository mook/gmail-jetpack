...
const Cookie = exports.Cookie = Class({
    ...
    toString: function() {
        let result = this.requestString;
        if (has(this, "domain")) {
            result += ";Domain=" + this.domain;
        }
        if (has(this, "path") && this.path != "/") {
        ...
        if (has(this, "expires")) {
            result += ";Expires=" + (new Date(this.expires)).toGMTString();
        }
        return result;
    },
    ...
    /**
     * Get the cookies as a string in the outgoing request
     * i.e. name/value pairs
     */
    get requestString() [this.name, this.value].map(escape).join("="),
});
